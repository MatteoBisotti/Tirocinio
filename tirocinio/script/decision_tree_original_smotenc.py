"""
    Questo script carica il dataset originale, applica oversampling tramite l'algoritmo SMOTENC.
    Applica un modello di albero di decisione, eseguendo un metodo di holdout sui dati per la 
...
file_handler = logging.FileHandler('../logs/dt_original_data_smotenc.log')
...
def main():
    ...
    dataset_augmented = pd.read_csv("../csv/dataset_SMOTENC.csv")

    dataset = func.drop_cols(dataset)
    dataset_augmented = func.drop_cols(dataset_augmented)
